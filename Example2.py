...
def schitalocka(kv_num = 1, count_p = 1, count_kv = 1):
    if kv_num and count_kv and count_p:
        if kv_num < count_p * count_kv:
            podiezd = 1
        else:
            podiezd = kv_num / (count_p * count_kv)
            if(podiezd > int(podiezd)):
                podiezd = int(podiezd) + 1
        podiezd = int(podiezd)
...
while True:
    ...
    match (your_choice):
        ...
        case 2:
            ...
            file_sum(name_f)
        case 3:
            print("Завдання Квартира.")
            kvart_number = int(input("Введіть номер квартири "))
            etazh_count = int(input("Введіть кількість поверхів "))
            kvart_count = int(input("Введіть кількість квартир "))
            schitalocka(kvart_number, etazh_count, kvart_count)
        case 4:
            print("Завдання складні відсотки.")
